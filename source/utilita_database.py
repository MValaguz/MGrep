...
def killa_sessione(p_sid,
    ...
    """
    if wx.MessageBox( message="Do you want to kill the selected session?", caption="Kill session", style=wx.YES_NO|wx.ICON_INFORMATION) == wx.YES:
        try:
        ...
        if v_ok:
            ...
            v_cursor.execute("ALTER SYSTEM KILL SESSION '" + str(p_sid) + "," + str(p_serial) + "'")
            v_cursor.close()
            v_connection.close()
            wx.MessageBox(message='The session is being closed.', caption='Kill session', style=wx.OK|wx.ICON_INFORMATION)
...
if __name__ == "__main__":
    ...
    v_sqlite_conn = sqlite3.connect(database='C:\SmiGrep\SmiGrepTransfer.db')
